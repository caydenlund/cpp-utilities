...
namespace util {
    ...
    args::args(const std::vector<std::string>& options_with_arguments)
        : _options_with_arguments(options_with_arguments.begin(), options_with_arguments.end()) {}

    std::string args::operator[](unsigned int index) const {
        if (index > _operands.size()) throw std::runtime_error("Out-of-bounds argument access");
        return _operands[index];
    }

    unsigned int args::operator[](const std::string& option) const {
        return _option_counts.contains(option) ? _option_counts.at(option) : 0;
    }

    std::vector<std::string> args::operator()(const std::string& option) const {
        return _option_arguments.contains(option) ? _option_arguments.at(option) : std::vector<std::string>();
    }

    std::vector<std::string>::const_iterator args::begin() const { return _operands.cbegin(); }

    std::vector<std::string>::const_iterator args::end() const { return _operands.cend(); }

    void args::parse(unsigned int argc, const char* const* argv) {
        ...
        for (unsigned int index = 1; index < argc; ++index) {
            ...
            if (argument[1] == '-') {
                ...
                enum scanner_state : uint8_t { START, VALID_SO_FAR, FOUND_EQUALS, VALID, INVALID };
                ...
                while (true) {
                    ...
                    switch (state) {
                        ...
                        case VALID_SO_FAR:
                            ...
                            if (!current_char_is_valid && current_char != '_' && current_char != '-') state = INVALID;
                            continue;
            ...
            //  We scan through the string to check for validity.
            enum scanner_state : uint8_t { VALID_SO_FAR, FOUND_OPT_ARG, VALID, INVALID };
    ...
    size_t args::size() const { return _operands.size(); }

    std::string args::name() const { return _name; }
